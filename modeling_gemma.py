...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if len(kv_cache.key_cache) <= self.layer_idx and Sparse_VLM :
            if self.layer_idx == 0:
                print()
                #print(f'Pre rotary_emb')
                print()
        ...
        if len(kv_cache.key_cache) <= self.layer_idx and Sparse_VLM :
            if self.layer_idx == 0:
                ...
                print(f'key_states_shape :- {key_states.shape}')
                print()

                print()
                print(f'value_states :- {value_states}')
                print(f'value_states_shape :- {value_states.shape}')
                print()

                print()
                print(f'query_states :- {query_states}')
                print(f'query_states_shape :- {query_states.shape}')
                print()
        """
        ...
        if len(kv_cache.key_cache) == 1 and Sparse_VLM :

                print()
                print(f'Pre-Softmax')
                print()
                print(f'attn_weights :- {attn_weights}')
                print(f'attn_weights_shape :- {attn_weights.shape}')
                print()
...
class GemmaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if len(kv_cache.key_cache) <= self.layer_idx and Sparse_VLM :
            if self.layer_idx == 0:
                print()
                print(f'Pre_attention_hidden_states :- {hidden_states}')
                print(f'Pre_attention_hidden_states_shape :- {hidden_states.shape}')
                print()
        ...
        if Sparse_VLM :
            if self.layer_idx == 0:
                print()
                print(f'Post_attention_hidden_states :- {hidden_states}')
                print(f'Post_attention_hidden_states_shape :- {hidden_states.shape}')
                print()
...
class GemmaModel(nn.Module):
    ...
    def forward(
        self,
        vision_token_pruning_percentage,
        text_token_pruning_percentage,
        text_mask,
        image_mask,
        Sparse_VLM,
        Diff_pruining_ratio_Decoder,
        dic, 
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        ...
        hidden_states_mask = (hidden_states != 0).float()
        print(f'hidden_states_mask :- {hidden_states_mask}')
        """
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(
        self, text_token_pruning_percentage, Sparse_VLM, image_features: torch.Tensor, inputs_embeds: torch.Tensor, input_ids: torch.Tensor, attention_mask: torch.Tensor, kv_cache: Optional[KVCache] = None
    ):
        ...
        return final_embedding, causal_mask, position_ids, text_mask, image_mask 
    ...
    def forward(
        ...
        inputs_embeds, attention_mask, position_ids, text_mask, image_mask = self._merge_input_ids_with_image_features(text_token_pruning_percentage, Sparse_VLM, image_features, inputs_embeds, input_ids, attention_mask, kv_cache)
